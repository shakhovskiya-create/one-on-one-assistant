...
// HELPER FUNCTIONS
// =====================
function createFrame(name: string, width: number, height: number): FrameNode {
...
function setFill(node: GeometryMixin, color: RGB, opacity: number = 1) {
  node.fills = [{ type: 'SOLID', color, opacity }];
}

function setStroke(node: GeometryMixin, color: RGB, weight: number = 1) {
...
function createText(content: string, fontSize: number, fontWeight: "Regular" | "Medium" | "SemiBold" | "Bold" = "Regular", color: RGB = COLORS.gray900): TextNode {
...
function createRoundedRect(name: string, width: number, height: number, radius: number, color: RGB): RectangleNode {
...
function createAvatar(name: string, initial: string, size: number, bgColor: RGB): FrameNode {
  const frame = createFrame(name, size, size);
  frame.cornerRadius = size / 2;
  setFill(frame, bgColor);

  const text = createText(initial, size * 0.4, "Medium", COLORS.white);
...
// Sidebar
function createSidebar(): FrameNode {
  const sidebar = createFrame("Sidebar", 256, 800);
  ...
  // Logo section
  const logoSection = createFrame("Logo Section", 256, 64);
  ...
  // EKF Logo box
  const logoBox = createRoundedRect("Logo Box", 48, 32, 4, COLORS.ekfRed);
  const logoText = createText("EKF", 16, "Bold", COLORS.white);

  const logoFrame = createFrame("Logo", 48, 32);
  ...
  const hubText = createText("Hub", 18, "SemiBold", COLORS.white);

  logoSection.appendChild(logoFrame);
  logoSection.appendChild(hubText);

  // Divider
  const divider = createRoundedRect("Divider", 256, 1, 0, COLORS.gray700);

  // Navigation
  const nav = createFrame("Navigation", 256, 600);
  ...
  menuItems.forEach(item => {
    const menuItem = createFrame(`Menu Item - ${item.label}`, 224, 44);
    ...
    // Icon placeholder
    const iconPlaceholder = createRoundedRect("Icon", 20, 20, 2, item.active ? COLORS.white : COLORS.gray300);
    iconPlaceholder.opacity = 0.5;

    const labelText = createText(item.label, 14, "Regular", item.active ? COLORS.white : COLORS.gray300);
...
// Header
function createHeader(title: string = "Задачи"): FrameNode {
  const header = createFrame("Header", 1200, 56);
  ...
  const titleText = createText(title, 20, "SemiBold", COLORS.gray800);

  const userSection = createFrame("User Section", 200, 40);
  ...
  const userName = createText("Антон Саховский", 14, "Medium", COLORS.gray700);
  const avatar = createAvatar("User Avatar", "А", 32, COLORS.ekfRed);
...
// Button
function createButton(label: string, variant: "primary" | "secondary" = "primary", size: "sm" | "md" = "md"): FrameNode {
  const height = size === "sm" ? 32 : 40;
  const fontSize = size === "sm" ? 13 : 14;
  const paddingX = size === "sm" ? 12 : 16;

  const button = createFrame(`Button - ${label}`, 100, height);
  ...
  const text = createText(label, fontSize, "Medium", variant === "primary" ? COLORS.white : COLORS.gray600);
...
// Input Field
function createInput(placeholder: string = "Поиск...", width: number = 200): FrameNode {
  const input = createFrame("Input", width, 36);
  ...
  const text = createText(placeholder, 14, "Regular", COLORS.gray400);
...
// Select/Dropdown
function createSelect(label: string = "Все проекты", width: number = 150): FrameNode {
  const select = createFrame("Select", width, 36);
  ...
  const text = createText(label, 14, "Regular", COLORS.gray700);
  const arrow = createText("▼", 10, "Regular", COLORS.gray400);
...
function createTaskCard(
  ...
  const card = createFrame(`Task Card - ${taskId}`, 260, 120);
  card.cornerRadius = 8;
  ...
  // Header row
  const headerRow = createFrame("Header Row", 236, 20);
  headerRow.layoutMode = "HORIZONTAL";
  headerRow.primaryAxisAlignItems = "SPACE_BETWEEN";
  headerRow.counterAxisAlignItems = "CENTER";
  headerRow.fills = [];

  const idText = createText(taskId, 11, "Regular", COLORS.gray400);

  if (storyPoints) {
    const spBadge = createFrame("SP Badge", 40, 18);
    ...
    const spText = createText(`${storyPoints} SP`, 10, "Medium", COLORS.gray600);
  ...
  // Title
  const titleText = createText(title, 13, "Medium", COLORS.gray900);
  titleText.layoutAlign = "STRETCH";
  titleText.textAutoResize = "HEIGHT";

  // Footer row
  const footerRow = createFrame("Footer Row", 236, 24);
  ...
  // Tags area
  const tagsArea = createFrame("Tags", 100, 20);
  tagsArea.fills = [];

  // Assignee
  if (assigneeInitial) {
    const avatar = createAvatar("Assignee", assigneeInitial, 24, COLORS.ekfRed);
...
function createKanbanColumn(
  ...
  const column = createFrame(`Column - ${name}`, 280, 600);
  ...
  // Header
  const header = createFrame("Column Header", 280, 48);
  header.layoutMode = "HORIZONTAL";
  ...
  const leftSection = createFrame("Left", 150, 24);
  leftSection.layoutMode = "HORIZONTAL";
  leftSection.itemSpacing = 8;
  leftSection.counterAxisAlignItems = "CENTER";
  leftSection.fills = [];

  const dot = createRoundedRect("Dot", 8, 8, 4, dotColor);
  const nameText = createText(name, 13, "Medium", COLORS.gray800);

  const countBadge = createFrame("Count", 28, 20);
  ...
  const countText = createText(count.toString(), 11, "Medium", COLORS.gray600);
  ...
  const spText = createText(`${storyPoints} SP`, 11, "Regular", COLORS.gray500);

  header.appendChild(leftSection);
  header.appendChild(spText);

  // Cards container
  const cardsContainer = createFrame("Cards", 280, 540);
...
// Filter Bar
function createFilterBar(): FrameNode {
  const filterBar = createFrame("Filter Bar", 1150, 56);
  ...
  // Search
  const search = createInput("Поиск задач...", 200);

  // Filters
  const projectFilter = createSelect("Все проекты", 140);
  const assigneeFilter = createSelect("Все исполнители", 150);
  const sprintFilter = createSelect("Все спринты", 140);
  const releaseFilter = createSelect("Все релизы", 120);

  // Spacer
  const spacer = createFrame("Spacer", 200, 1);
  spacer.layoutGrow = 1;
  spacer.fills = [];

  // View toggle
  const viewToggle = createFrame("View Toggle", 160, 36);
  viewToggle.cornerRadius = 8;
  setStroke(viewToggle, COLORS.gray200);
  viewToggle.layoutMode = "HORIZONTAL";
  viewToggle.fills = [];

  const listBtn = createFrame("List Btn", 80, 36);
  ...
  const listText = createText("Список", 13, "Regular", COLORS.gray600);
  listBtn.appendChild(listText);

  const kanbanBtn = createFrame("Kanban Btn", 80, 36);
  ...
  const kanbanText = createText("Kanban", 13, "Medium", COLORS.white);
...
async function generateTasksPage(): Promise<FrameNode> {
  const page = createFrame("Tasks Page", 1440, 900);
  ...
  // Main content area
  const mainContent = createFrame("Main Content", 1184, 900);
  mainContent.layoutMode = "VERTICAL";
  mainContent.fills = [];

  // Header
  const header = createHeader("Задачи");
  header.layoutAlign = "STRETCH";

  // Content
  const content = createFrame("Content", 1184, 844);
  ...
  // Page title row
  const titleRow = createFrame("Title Row", 1152, 48);
  titleRow.layoutMode = "HORIZONTAL";
  titleRow.primaryAxisAlignItems = "SPACE_BETWEEN";
  titleRow.counterAxisAlignItems = "CENTER";
  titleRow.fills = [];

  const titleSection = createFrame("Title Section", 400, 48);
  titleSection.layoutMode = "VERTICAL";
  titleSection.itemSpacing = 4;
  titleSection.fills = [];

  const pageTitle = createText("Задачи", 20, "Bold", COLORS.gray900);

  const statsRow = createFrame("Stats", 300, 20);
  statsRow.layoutMode = "HORIZONTAL";
  statsRow.itemSpacing = 12;
  statsRow.fills = [];
  const stat1 = createText("24 задачи", 13, "Regular", COLORS.gray500);
  const stat2 = createText("89 SP", 13, "Medium", COLORS.blue600);
  ...
  const createBtn = createButton("Создать", "primary", "sm");
  ...
  // Kanban board
  const kanbanBoard = createFrame("Kanban Board", 1152, 600);
  kanbanBoard.layoutMode = "HORIZONTAL";
  kanbanBoard.itemSpacing = 16;
  kanbanBoard.fills = [];

  // Columns
  const backlogCol = createKanbanColumn("Backlog", 4, 12, COLORS.gray100, COLORS.gray400);
  const analysisCol = createKanbanColumn("Анализ", 2, 8, COLORS.purple50, COLORS.purple400);
  const devCol = createKanbanColumn("Разработка", 3, 21, COLORS.blue50, COLORS.blue500);
  const reviewCol = createKanbanColumn("Review", 1, 5, COLORS.yellow50, COLORS.yellow400);
  const testCol = createKanbanColumn("Тестирование", 2, 8, COLORS.orange50, COLORS.orange400);
  const doneCol = createKanbanColumn("Готово", 12, 35, COLORS.green50, COLORS.green500);
...
async function generateComponentLibrary(): Promise<FrameNode> {
  const library = createFrame("EKF UI Components", 1200, 2000);
  ...
  // Title
  const title = createText("EKF Hub UI Kit", 32, "Bold", COLORS.gray900);
  library.appendChild(title);

  // Colors section
  const colorsSection = createFrame("Colors", 1120, 200);
  colorsSection.layoutMode = "VERTICAL";
  colorsSection.itemSpacing = 16;
  colorsSection.fills = [];

  const colorsTitle = createText("Colors", 20, "SemiBold", COLORS.gray800);
  colorsSection.appendChild(colorsTitle);

  const colorSwatches = createFrame("Color Swatches", 1120, 60);
  ...
  colorList.forEach(c => {
    const swatch = createFrame(c.name, 100, 60);
    swatch.layoutMode = "VERTICAL";
    swatch.itemSpacing = 4;
    swatch.fills = [];

    const colorBox = createRoundedRect("Color", 100, 40, 8, c.color);
    const label = createText(c.name, 10, "Regular", COLORS.gray600);
  ...
  // Buttons section
  const buttonsSection = createFrame("Buttons", 1120, 100);
  buttonsSection.layoutMode = "VERTICAL";
  buttonsSection.itemSpacing = 16;
  buttonsSection.fills = [];

  const buttonsTitle = createText("Buttons", 20, "SemiBold", COLORS.gray800);
  buttonsSection.appendChild(buttonsTitle);

  const buttonRow = createFrame("Button Row", 600, 50);
  ...
  // Inputs section
  const inputsSection = createFrame("Inputs", 1120, 100);
  inputsSection.layoutMode = "VERTICAL";
  inputsSection.itemSpacing = 16;
  inputsSection.fills = [];

  const inputsTitle = createText("Inputs & Selects", 20, "SemiBold", COLORS.gray800);
  inputsSection.appendChild(inputsTitle);

  const inputRow = createFrame("Input Row", 600, 50);
  ...
  // Task Cards section
  const cardsSection = createFrame("Task Cards", 1120, 200);
  cardsSection.layoutMode = "VERTICAL";
  cardsSection.itemSpacing = 16;
  cardsSection.fills = [];

  const cardsTitle = createText("Task Cards", 20, "SemiBold", COLORS.gray800);
  cardsSection.appendChild(cardsTitle);

  const cardsRow = createFrame("Cards Row", 1100, 140);
  ...
  // Sidebar
  const sidebarSection = createFrame("Sidebar", 1120, 500);
  sidebarSection.layoutMode = "VERTICAL";
  sidebarSection.itemSpacing = 16;
  sidebarSection.fills = [];

  const sidebarTitle = createText("Sidebar", 20, "SemiBold", COLORS.gray800);
...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.type === 'generate-tasks-page') {
    const page = await generateTasksPage();
  ...
  if (msg.type === 'generate-task-card') {
    const card = createTaskCard("TASK-001", "Example Task", "medium", 5, "А");
  ...
  if (msg.type === 'generate-kanban-column') {
    const column = createKanbanColumn("Backlog", 4, 12, COLORS.gray100, COLORS.gray400);
