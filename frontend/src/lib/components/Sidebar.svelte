<script lang="ts">
	...
	// Context-aware sidebar items based on current module
	const tasksSidebarItems = [
		{ href: '/tasks', label: 'Доска задач', icon: 'board', group: 'planning' },
		{ href: '/projects', label: 'Проекты', icon: 'folder', group: 'planning' },
		{ href: '/sprints', label: 'Спринты', icon: 'sprint', group: 'sprints' },
		{ href: '/releases', label: 'Релизы', icon: 'release', group: 'releases' },
	];

	const meetingsSidebarItems = [
		{ href: '/meetings', label: 'Календарь', icon: 'calendar', group: 'overview' },
		{ href: '/calendar', label: 'Расписание', icon: 'schedule', group: 'overview' },
	];

	const defaultSidebarItems = [
		{ href: '/messenger', label: 'Сообщения', icon: 'messenger', group: 'communication' },
		{ href: '/mail', label: 'Почта', icon: 'mail', group: 'communication' },
		{ href: '/confluence', label: 'Confluence', icon: 'book', group: 'docs' },
		{ href: '/github', label: 'GitHub', icon: 'github', group: 'docs' },
	...
	function getSidebarItems() {
		...
		if (path.startsWith('/tasks') || path.startsWith('/projects') || path.startsWith('/sprints') || path.startsWith('/releases')) {
		...
		return { items: defaultSidebarItems, title: 'Инструменты' };
	}
...
<div class="h-full flex flex-col">
	...
	<nav class="flex-1 p-2 overflow-y-auto">
		<ul class="space-y-1">
			{#each sidebarConfig.items as item}
				<li>
					<a
						href={item.href}
