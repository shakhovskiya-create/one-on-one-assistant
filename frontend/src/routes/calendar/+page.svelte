<script lang="ts">
	...
	import { calendar as calendarApi, employees as employeesApi } from '$lib/api/client';
	import type { Employee, CalendarEvent } from '$lib/api/client';

	let employees: Employee[] = $state([]);
	let selectedEmployee: string = $state('');
	...
	async function loadCalendar() {
		...
		loading = true;
		try {
			events = await calendarApi.getSimple(selectedEmployee);
		} catch (e) {
			console.error(e);
			events = [];
		} finally {
	...
	<div class="flex items-center justify-between">
		...
		<div class="flex items-center gap-4">
			<select
				...
				class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-ekf-red focus:border-transparent"
			>
			...
			<button
				...
				disabled={syncing}
				class="px-4 py-2 bg-ekf-red text-white rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50"
			>
				{#if syncing}
					Синхронизация...
				{:else}
